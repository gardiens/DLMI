...
def apply_tta(sample,num_tta:int,tta_transform):
    ...
    return output
def test_model_TTA(model,test_dataset,device,BATCH_SIZE,test_ids,test_dataloader,name_out="baseline.csv",combine_strategy="mean",tta_strategy="simple",threshold=0.5,clearml_found=False):
    model=model.eval() 
  
    predictions = []
    tta_transform=get_transform_tta(tta_strategy)
    for sample,_ in tqdm.tqdm(test_dataset):
        print("the input",sample.shape)
        with torch.no_grad(): 
            sample=sample.to(device)
            sample_tta=apply_tta(sample,num_tta=4,tta_transform=tta_transform)
